...
char **mx_unic_arr(st_main *main, int size) {
	...
	main->unic_temp = (char**)malloc(sizeof(char**) * 10000);
	for (int i = 0, j = 0; main->g_mass[i] != NULL; i++) {
		main->unic_temp[n] = (char*)malloc(sizeof(char*) * 200);	
		for (;main->g_mass[i][j] != '-'; j++)
			count++;
		mx_strncpy(main->unic_temp[n], main->g_mass[i], count);
		n++;
		main->unic_temp[n] = (char*)malloc(sizeof(char*) * 40);
		j += 1;
		count = 0;
		cirlce(main, i, j, n);
		n++;
